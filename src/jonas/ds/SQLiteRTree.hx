...
// compilter was getting lost with empty: Void -> Bool and empty: jonas.Maybe<A>
...
class SQLiteRTree<T> implements RTree<T> {
	...
	public function insertPoint( x: Float, y: Float, object: T ): Void {
		...
		length++;
		jonas.macro.Error.throwIf( cnx.request( 'SELECT count( 1 ) cnt FROM tree' ).next().cnt != length );
	}
